...
void release( int t ) 
{
	...
	{ 	//如果该子树都被加上的一个数。
		...
		node[t].cot += node[t].size * node[t].mark;
...
{ //维护t的信息。
	...
	node[t].cot = node[t].data + node[node[t].ch[0]].cot + node[node[t].ch[1]].cot;
}
...
void solve() 
{
	...
	for ( int i = 1 ; i <= m ; i++ ) {
		...
		else if(strcmp(str, "Q") == 0)
		{
			...
			printf("%lld\n", node[t].cot);
			//printf( "cot:%lld  min:%d\n" , node[t].cot, node[t].mins ); //基本同ADD操作，而这里是获取最小值的信息。
		}
